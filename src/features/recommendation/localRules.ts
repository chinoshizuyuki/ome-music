import type { RecommendationContext, RecommendationItem, Track, UserMusicProfile } from "../../types/music";
...
export function buildLocalRecommendations(
  ...
  const repeatedLabels = new Set(profile.repeatPatterns.slice(0, 3).map((pattern) => pattern.label));
  ...
  return tracks
    .map((track) => {
      ...
      const affinityScore = track.liked ? 0.3 : repeatedLabels.has(track.artist) || repeatedLabels.has(track.album) ? 0.22 : 0.08;
